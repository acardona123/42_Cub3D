...
/*   Updated: 2023/11/03 05:11:04 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	gp_action_do(t_general *gen)
{
	...
	t_chunk		*hit_chunk;
				...
				// printf("\n\e[103maction\e[0m\n---\n");//
	hit_point = r_ray_hit(gen, &gen->player.p_co, gen->player.p_angle, true);
	...
	if (hit_point.dist > hit_chunk->action->dist_range)
		return ;
	(*hit_chunk->action->execute)(gen, hit_chunk->action, hit_point.hit_face);
}
