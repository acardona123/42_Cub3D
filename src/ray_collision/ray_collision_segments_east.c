...
/*   Updated: 2023/11/07 15:26:38 by acardona         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @return t_hitpoint 
 */
t_hitpoint	r_ray_hit_n_ne(t_general *gen, t_hitpoint last, t_ray_data rdata)
{
							//printf("  first: %c\n", "HVA"[rdata.first]);//
	...
	while (++last.pt_co.y <= gen->map.y_max - 1. - rdata.shift)
	{
		...
		{
			if (++rdata.last_v.x > gen->map.x_max - 1. - rdata.shift)
				return (last.pt_co.x = -1., last);
			++last.chunk_co_x;
			rdata.last_v.y += rdata.delta_y;
			if (r_ray_hit_sec(gen, &last, &rdata, rdata.last_v))
				return (last.hit_face = FACE_W, last.pt_co.x -= EPSILON, last);
		}
	}
	return (last.pt_co.x = -1., last);
}
 ...
 * @return t_hitpoint 
 */
t_hitpoint	r_ray_hit_ne_e(t_general *gen, t_hitpoint last, t_ray_data rdata)
{
							//printf("  first: %c\n", "HVA"[rdata.first]);
							//printf("- before: last: (%f, %f)\n", last.pt_co.x, last.pt_co.y);//
							//printf("  before: chunk_co: (%d, %d)\n", last.chunk_co_x, last.chunk_co_y);//
	...
	while (++last.pt_co.x <= gen->map.x_max - 1. - rdata.shift)
	{
		++last.chunk_co_x;
								//printf("- begin last: (%f, %f)\n", last.pt_co.x, last.pt_co.y);//
								//printf("  begin chunk_co: (%d, %d)\n", last.chunk_co_x, last.chunk_co_y);//
 ...
 * @return t_hitpoint 
 */
t_hitpoint	r_ray_hit_e_se(t_general *gen, t_hitpoint last, t_ray_data rdata)
{
							//printf("  first: %c\n", "HVA"[rdata.first]);//
							//printf("- before: last: (%f, %f)\n", last.pt_co.x, last.pt_co.y);//
							//printf("  before: chunk_co: (%d, %d)\n", last.chunk_co_x, last.chunk_co_y);//
 ...
 * @return t_hitpoint 
 */
t_hitpoint	r_ray_hit_se_s(t_general *gen, t_hitpoint last, t_ray_data rdata)
{
							//printf("  first: %c\n", "HVA"[rdata.first]);//
							//printf("- before: last: (%f, %f)\n", last.pt_co.x, last.pt_co.y);//
							//printf("  before: chunk_co: (%d, %d)\n", last.chunk_co_x, last.chunk_co_y);//
	...
	while (--last.pt_co.y >= 1. + rdata.shift)
	{
		...
		if (rdata.check_v && last.pt_co.x - rdata.last_v.x >= 1.)
		{
			...
			rdata.last_v.y += rdata.delta_y;
			if (r_ray_hit_sec(gen, &last, &rdata, rdata.last_v))
				return (last.pt_co.x -= EPSILON, last.hit_face = FACE_W, last);
		}
	}
	return (last.pt_co.x = -1., last);
}
